<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100]">
    ...
    <div class="relative flex items-center justify-center min-h-screen p-4">
      <div class="relative bg-base-100 rounded-lg shadow-2xl w-full max-w-6xl border border-base-300">
        <!-- Header -->
        ...
        <!-- Content -->
        <div class="bg-base-100 rounded-b-lg" :class="activeTab === 'dependencies' ? 'p-2' : 'p-6'">
          <!-- Keyboard Shortcuts -->
          ...
          <!-- Archive System -->
          <div v-else-if="activeTab === 'dependencies'" class="h-[70vh]">
            <div class="flex items-center justify-center h-full text-center">
              <div>
                <GitBranch class="w-16 h-16 mx-auto mb-4 text-base-content/40" />
                <h3 class="text-lg font-semibold mb-2">Archive Data Management</h3>
                <p class="text-base-content/70">Use the main help panel (?) to access the archive management system</p>
              </div>
            </div>
          </div>
        </div>
...
import { ref } from 'vue';
import { X, Keyboard, MousePointer, Sparkles, GitBranch } from 'lucide-vue-next';
